-- ============================================================================
-- Division-aware chips on search results.
--
-- Tournament events can carry multiple divisions of varying surface, format,
-- gender, and skill tier. Today the `search_events` RPC projects these as
-- single scalars derived from the *primary* (lowest sort_order) division,
-- which forces card UIs to either lie ("Coed sixes BB") or show only the
-- primary division's attributes. This migration:
--
--   1. Extends the `divisions` jsonb array returned per event to include
--      `surface`, `format`, `gender` so the client can render per-division
--      chips and roll them up into "varies / N divisions" copy.
--   2. Switches the `p_format` / `p_gender` / `p_skill_level` filters to
--      match if ANY division on the event matches (instead of just the
--      primary). This makes a tournament with a Men's BB division show up
--      under both "Men's" and "BB" filters.
--
-- Open-play events still have exactly one division (synthesized at create
-- time) so behavior is unchanged for them.
-- ============================================================================
