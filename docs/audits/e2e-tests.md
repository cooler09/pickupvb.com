...
refund** (near-future `pickNearFutureDateTime` helper), **Julie** 30d cap
(stateful + needs a Stripe-Connect free-tier host — Carlos may fit better), and
**Nina** publish-after-onboarding (in-test Connect onboarding — hardest).

**Remaining persona `test.fixme` backlog (14).** Grouped by what's actually
...
| Julie   | 2nd paid event in 30d blocked by the cap                         | **Stripe fixture**                   | `validateHostPaidEventCap` is checked at creation (no checkout), but the test is **stateful + host-state-dependent**: Julie must be Stripe-Connect-onboarded (free tier) AND have no paid event in the rolling 30d at run start, then create #1 (real Stripe event) so #2 is blocked. Carlos (P6, "Stripe-onboarded but free tier") may be the better-fitting actor.                                                                                        |
