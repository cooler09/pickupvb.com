...
> **Status (2026-05-23, Bundle 40):** **Onboarding guide landed.** New [docs/onboarding.md](../onboarding.md) is a thin overlay on the existing setup docs — a 15-minute TL;DR (clone → install → local Supabase → run → verify quad), a prioritised reading order (AGENTS.md first; per-layer READMEs; ADRs / api-reference / testing / DB-ops / monitoring / runbook on demand), a five-bullet mental model (hexagonal, typed errors, page composition, anonymous auth, revalidate-on-mutate), a first-PR walkthrough, and a troubleshooting table for the common stuck-on-day-1 failures (Docker, migrate, type regen, `exactOptionalPropertyTypes` JSX, missing `revalidatePath`). Deliberately doesn't duplicate README's getting-started or `setup-dev-environment.md`'s staging provisioning — it links into them. P3 onboarding finding flipped ✅. See the [Bundle 40 journal](../journal/2026-05-23-bundle-40.md).
>
...
### No developer onboarding guide ✅ (2026-05-23, Bundle 40)
...
- **Resolved (Bundle 40):** [docs/onboarding.md](../onboarding.md) authored as a thin overlay rather than a standalone setup guide — README already covers prerequisites + install + local Supabase, and [setup-dev-environment.md](../setup-dev-environment.md) covers staging provisioning. The onboarding doc adds the missing pieces: a 15-minute TL;DR block, **reading order** (AGENTS.md → README → per-layer READMEs, then ADRs / api-reference / testing / DB-ops / monitoring / runbook on demand), a five-bullet mental model, a first-PR walkthrough centred on the verify quad, and a troubleshooting table for the day-1 stuck states (Docker not running, migration partway, missing Supabase types, `exactOptionalPropertyTypes` JSX traps, missing `revalidatePath`).
...
| 2026-05-23 | P3: developer onboarding guide          | ✅ Fixed   | [docs/onboarding.md](../onboarding.md) authored as a thin overlay on the existing setup docs rather than a standalone guide (README already has prereqs + install + local Supabase; [setup-dev-environment.md](../setup-dev-environment.md) covers staging). New material: 15-minute TL;DR (clone → install → Supabase local → run → verify quad), prioritised reading order (AGENTS.md first, then per-layer READMEs, then ADRs/api-reference/testing/DB-ops/monitoring/runbook on demand), a five-bullet mental model (hexagonal layering, typed `DomainError`, page composition under `_components/`, anonymous-auth claim, `revalidatePath()` after mutating actions), a first-PR walkthrough that puts the verify quad at the centre, and a troubleshooting table for the common day-1 stuck states (Docker, partway migration, missing Supabase types, `exactOptionalPropertyTypes` JSX traps, server-action 404 from raw `formData.get`, missing `revalidatePath`).   |
...
- **P3: ~~Migration preamble standard~~, ~~onboarding guide~~, CHANGELOG, flow diagrams, code-of-conduct.** (Migration preamble standard closed by Bundle 39, 2026-05-22; onboarding guide closed by Bundle 40, 2026-05-23.)
