...
> **Status (2026-05-23, Bundle 59):** **P3 "TypeScript packages don't use `composite` / project references" closed as Wontfix-by-design.** The audit itself flagged the win as "marginal at current scale" and prescribed "revisit if build times become painful." Six months on, build times haven't become painful — `pnpm build` reports 8/8 turbo tasks in seconds on a warm cache and a clean-cache build is still well under a minute. Adopting `composite: true` + cross-package `references` would force every package to emit `.tsbuildinfo` files (managing a new ignore surface) and force every `tsconfig.json` to carry an explicit `references` block kept in sync with `dependencies` — ongoing maintenance for a speedup we can't measure. Trigger to re-open: clean-cache `pnpm build` exceeds ~60s or `pnpm typecheck` exceeds ~15s sustained. See the [Bundle 59 journal](../journal/2026-05-23-bundle-59.md).
>
> **Status (2026-05-23, Bundle 58):** **P3 "CI uses hardcoded Supabase placeholder keys" closed as Wontfix-by-design.** The placeholders are deliberate — `next build` requires `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` to be present (Supabase browser client throws on import otherwise) but doesn't issue real requests during the build, so a fake value satisfies the gate. Real-Supabase validation runs in [.github/workflows/smoke-prod.yml](../../.github/workflows/smoke-prod.yml) post-deploy via Playwright against the live URL — the place where any actual schema or auth regression would surface. Wiring a real CI Supabase would burn shared quota and require a service-role secret without catching anything the post-deploy smoke doesn't already catch. An explanatory comment on the `Build` step in [.github/workflows/ci.yml](../../.github/workflows/ci.yml) now documents the choice so the next reader doesn't try to "fix" it. See the [Bundle 58 journal](../journal/2026-05-23-bundle-58.md).
>
> **Status (2026-05-22, Bundle 38):** **P2 issue templates + CODEOWNERS closed.** Bug-report template had already landed (out-of-band, not previously logged); this bundle adds [.github/ISSUE_TEMPLATE/feature-request.yml](../../.github/ISSUE_TEMPLATE/feature-request.yml) (problem-not-solution framing, product-surface dropdown matching the app's top-level sections, who-benefits prompt) and a catch-all [.github/CODEOWNERS](../../.github/CODEOWNERS) routing every path to `@cooler09` with a header note to narrow the globs once additional regular contributors land. See the [Bundle 38 journal](../journal/2026-05-22-bundle-38.md).
>
...
| TS per-package        | `packages/*/tsconfig.json`                                           | ✅ extends base; no project refs / composite (Wontfix-by-design, Bundle 59) |
...
| Issue templates       | ✅ added 2026-05-22 (bug-report + feature-request)                   |
| CODEOWNERS            | ✅ added 2026-05-22 (Bundle 38, catch-all to `@cooler09`)            |
...
### No PR template, issue templates, or CODEOWNERS ✅ (2026-05-22, Bundle 38)
...
- **Resolved (Bundle 38):** All three surfaces now wired. PR template landed 2026-05-17. Bug-report issue template ([.github/ISSUE_TEMPLATE/bug-report.yml](../../.github/ISSUE_TEMPLATE/bug-report.yml)) + the `config.yml` contact-link to support email had landed out-of-band (not previously logged in this audit). Bundle 38 adds [feature-request.yml](../../.github/ISSUE_TEMPLATE/feature-request.yml) (problem-first framing, product-surface dropdown, who-benefits prompt) and a minimal catch-all [CODEOWNERS](../../.github/CODEOWNERS) (`* @cooler09`) so GitHub's code-review-assignment surface is wired without pretending the project has finer ownership boundaries than it does. CODEOWNERS header explicitly calls out the narrowing strategy for when additional regular contributors land.
...
### TypeScript packages don't use `composite` / project references ✅ Closed Wontfix-by-design (2026-05-23, Bundle 59)
...
- **Resolved:** Left as-is by design. Verify-quad turbo tasks (typecheck / lint / test / build) complete in seconds on a warm cache and a clean-cache `pnpm build` is still well under a minute — the audit's own "revisit if painful" threshold hasn't been crossed. Composite mode adds ongoing maintenance (`.tsbuildinfo` files to ignore, `references` arrays in every `tsconfig.json` kept in sync with `dependencies`) for a speedup we can't currently measure. Trigger to re-open: clean-cache `pnpm build` exceeds ~60s or `pnpm typecheck` exceeds ~15s sustained.
...
### CI uses hardcoded Supabase placeholder keys ✅ Closed Wontfix-by-design (2026-05-23, Bundle 58)
...
- **Resolved:** Left as-is by design. The placeholders satisfy `next build`'s import-time validation of the Supabase browser-client env vars without issuing real requests during the build, so they catch the "forgot to set the env var" class of regression without burning shared Supabase quota. The actual-Supabase smoke is intentionally handled by [.github/workflows/smoke-prod.yml](../../.github/workflows/smoke-prod.yml) running Playwright against the deployed URL — see [docs/monitoring.md](../monitoring.md) and [docs/testing.md](../testing.md). The CI comment block (Bundle 58) makes the choice explicit so the next contributor doesn't "helpfully" swap in a real key.
...
- **CI Supabase smoke** — ~~is the placeholder-keys-only approach intentional (fast CI) or would a real CI Supabase be useful?~~ **Answered Bundle 58 (2026-05-23): intentional.** Placeholders satisfy `next build` import-time validation; the post-deploy [smoke-prod.yml](../../.github/workflows/smoke-prod.yml) covers real-Supabase regressions where they'd actually surface.
...
| 2026-05-22 | P2 — Issue templates + CODEOWNERS (Bundle 38)          | Closed the long-running P2. Bug-report template + `config.yml` contact link to `support@pickupvb.com` had landed out-of-band (not previously logged here). Bundle 38 adds the feature-request template (problem-first framing, product-surface dropdown matching app sections, who-benefits prompt) and a minimal catch-all CODEOWNERS routing every path to `@cooler09`. Header on CODEOWNERS explicitly calls out the narrowing strategy (give domain experts ownership of `packages/domain/`, infra folks ownership of `supabase/migrations/`) for when additional regular contributors land, so the catch-all isn't mistaken for the long-term shape. | [.github/ISSUE_TEMPLATE/feature-request.yml](../../.github/ISSUE_TEMPLATE/feature-request.yml), [.github/CODEOWNERS](../../.github/CODEOWNERS)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-05-23 | P3 — CI Supabase placeholder keys (Bundle 58)          | Closed as Wontfix-by-design. The placeholders satisfy `next build`'s import-time validation of `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (the browser client throws on import otherwise) without issuing real requests, and the actual-Supabase smoke is covered by `smoke-prod.yml` running Playwright against the deployed URL post-deploy. Wiring a real CI Supabase would burn shared quota and require a service-role secret without catching anything the post-deploy smoke doesn't. Added an explanatory comment on the `Build` step in `ci.yml` so the next reader doesn't "fix" it back to a real key.                 | [.github/workflows/ci.yml](../../.github/workflows/ci.yml)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 2026-05-23 | P3 — TypeScript composite / project refs (Bundle 59)   | Closed as Wontfix-by-design. The audit prescribed "revisit if build times become painful" — they haven't. Warm-cache verify quad runs in seconds; clean-cache `pnpm build` is comfortably under a minute. Composite mode would add `.tsbuildinfo` ignore surface and force every `tsconfig.json` to carry an explicit `references` array kept in sync with `dependencies` for an unmeasurable speedup at current scale. Re-open trigger documented: clean-cache `pnpm build` > ~60s or `pnpm typecheck` > ~15s sustained.                                                                                                                                 | `packages/*/tsconfig.json` (no edits — audit-doc closure only)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- ~~**P2** Issue templates + CODEOWNERS — only PR template shipped.~~ — **closed** by Bundle 38 (2026-05-22): bug-report + feature-request issue templates and a catch-all CODEOWNERS (`* @cooler09`) all in place; header on CODEOWNERS calls out the narrowing strategy for when contributors land.
...
- All **P3** items _except_ ~~CI Supabase placeholder keys~~ (closed Wontfix-by-design by Bundle 58, 2026-05-23) and ~~TypeScript composite / project references~~ (closed Wontfix-by-design by Bundle 59, 2026-05-23). Remaining P3s: Node 22 LTS bump, `CHANGELOG` / `changesets`.
- All **Open questions** above _except_ ~~CI Supabase smoke~~ (answered Bundle 58, 2026-05-23: intentional).
