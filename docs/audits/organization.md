...
- **P1** Test scripts are no-ops in most packages — cross-listed with architecture audit; pending test framework choice. Status update: `domain`, `application`, and `web` already run real Vitest suites; `infrastructure`, `notifications`, `supabase`, `types`, `config` still echo. Remaining gap is mostly infrastructure adapter tests.
