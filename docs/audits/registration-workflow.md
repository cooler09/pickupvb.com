...
_Last updated: 2026-05-27 (Bundle 121)_

> **Status (2026-05-27, Bundle 121):** **Free-division price-unit
> exemption.** The 2026-05-22 "Payment policy P1" remediation
> (`validateTeamPricing` + the matching domain invariant) was correct
> for paid divisions but over-applied: free divisions (`price_cents = 0`)
> were rejected for the same `(team-led + per_player)` shape even though
> no payment is routed. Both the domain rule
> (`VolleyballEvent.assertRegistrationConfigValid`) and the boundary
> mirror (`validateTeamPricing`) now skip the price-unit clause when
> `price_cents <= 0`; write boundaries normalize the persisted unit per
> mode; the "Charge" select is hidden on free divisions in both
> `DivisionsRepeater` and `HostDivisionsManager`. ADR 0012 amended with
> a "Free-division exemption" section. See remediation row below and
> [docs/journal/2026-05-digest.md#bundle-121](../journal/2026-05-digest.md#bundle-121).

> **Status (2026-05-27, Bundle 120):** **Model R4 (P3) — walk-in team
> registrations — closed.** ADR
> [0017](../adr/0017-walk-in-registrations.md) introduces a `source`
> discriminator (`captain` / `host` / `walk_in`) on
> `event_team_registrations`, a nullable `captain_id` with freeform
> `captain_display_name` + `captain_phone` for accountless captains,
> a new `markPaidCash` domain method that records a freeform
> `payment_note`, and a host UI ("Add walk-in team" form, "Mark paid
> (cash)" action, "Walk-in" pill on the public roster). Migration
> [20260712000000_walk_in_registrations.sql](../../supabase/migrations/20260712000000_walk_in_registrations.sql)
> adds the columns, the discriminant constraint, the source index,
> and rewrites the insert RLS policy to allow host inserts on walk-ins
> on published ad-hoc divisions. With R4 closed, **all six Bundle 117
> re-audit findings are now resolved.** See the Bundle 120 remediation
> row and journal entry
> [docs/journal/2026-05-digest.md#bundle-120](../journal/2026-05-digest.md#bundle-120).
>
> **Status (2026-05-27, Bundle 119):** **Model R3 (P2) — per-division
> `team_registration_mode` — closed.** ADR
> [0016](../adr/0016-per-division-team-registration-mode.md) supersedes
> ADR 0008 §2 and amends ADR 0012's matrix; migration
> [20260711000000_per_division_team_registration_mode.sql](../../supabase/migrations/20260711000000_per_division_team_registration_mode.sql)
> moves the column from `events` to `event_divisions`, backfills, drops
> the parent column, and rewrites the two team-insert RLS policies to
> join `event_divisions`. UI now renders both ad-hoc and roster
> sub-panels stacked when an event has divisions of both modes. Of the
> six Bundle 117 re-audit findings, only **R4 (walk-ins)** remains open
> (R5 — single-division UI collapse — tracked separately as
> originally logged). See the Bundle 119 remediation row and journal
> entry [docs/journal/2026-05-digest.md#bundle-119](../journal/2026-05-digest.md#bundle-119).
>
> **Status (2026-05-27, Bundle 117 re-audit):** Fresh pass requested by
> product after the model stabilized post-Bundle-68. New framing: **each
> division owns its registrations (teams + free agents); the event is
> just the container.** Re-traced schema + domain + UI against that
> framing. Most of the model is already division-aware (Bundles 1–5
> wired `division_id` explicitly through `attachTeamToDivision` /
> `attachFreeAgentToDivision`, and migration
> [20260606000000_team_registration_model.sql](../../supabase/migrations/20260606000000_team_registration_model.sql)
> made it NOT NULL on all three join tables) — but four real gaps
> surfaced, plus a UX cleanup for single-division tournaments. New
> findings are listed under **Re-audit findings (2026-05-27)** below
> and rolled into **Still open** at the bottom:
>
> - **Model R1 (P1)** — `event_free_agents` PK is `(event_id, user_id)`,
>   so a free agent can sign up for at most one division per event even
>   though `division_id` is NOT NULL. Conflicts with "divisions own
>   their free agents."
> - **Model R2 (P1)** — No host toggle for free-agent enrollment.
>   `event_free_agents` accepts inserts on every published tournament;
>   the host can't disable free agents on a division (or on the event).
>   User explicitly called this out as a missing knob.
> - **Model R3 (P2)** — `team_registration_mode` is **event-level**
>   (ADR 0008 single-mode-per-event). User asked for per-division mode
>   (e.g. "AA is rostered, BB is ad-hoc, walk-ins allowed everywhere").
>   Requires re-litigating ADR 0008 if pursued.
> - **Model R4 (P2)** — **Walk-in is not modeled.** User listed
>   walk-ins alongside ad-hoc and rostered as a first-class option;
>   currently the only ways to add a same-day team are (a) captain
>   creates an ad-hoc registration from their phone, or (b) host
>   creates an ad-hoc registration on the captain's behalf via
>   [host-team-registration-actions.ts](../../apps/web/src/app/events/%5Bid%5D/host-team-registration-actions.ts).
>   No distinct walk-in payment / identity / display path.
> - **UX R5 (P2)** — Single-division tournaments still render
>   `DivisionsSection` ("Divisions (1)" header + full card), duplicating
>   skill / format / price already in `EventHero`. The
>   `TeamsRegisteredSection` heading also doesn't elide the division
>   column on the ad-hoc roster on a single-division event.
> - **Model R6 (P3, architectural)** — `event_teams.event_id` and
>   `event_team_registrations.event_id` are redundant with
>   `division_id` (every division belongs to exactly one event); they
>   exist for read-path convenience but encode the "event owns teams"
>   model the user is moving away from. Optional cleanup.
>
> No code changes in Bundle 117 — this is the audit write-up only.
> Recommended sequencing: R2 + R5 are cheap product wins; R1 needs a
> PK migration; R3 + R4 are product-decision-first (likely an ADR
> 0008 amendment or successor); R6 is a refactor with no user-visible
> change. See the **Re-audit findings (2026-05-27)** section for fix
> sketches and file references.
...
> [Bundle 68 journal](../journal/2026-05-digest.md#bundle-68).
>
...
> [Bundle 63 journal](../journal/2026-05-digest.md#bundle-63).
>
...
> [Bundle 62 journal](../journal/2026-05-digest.md#bundle-62).
>
...
> [Bundle 55 journal](../journal/2026-05-digest.md#bundle-55).
>
...
> [Bundle 52 journal](../journal/2026-05-digest.md#bundle-52).
>
...
> [Bundle 7 journal](../journal/2026-05-digest.md#bundle-7). The full
...
>   See the [Bundle 4 journal](../journal/2026-05-digest.md#bundle-4) for the
...
## Re-audit findings (2026-05-27)

Framing: **the event is a container; each division owns its own teams
and (optionally) free agents.** Re-traced against today's tree.

### Model

#### R1 (P1) — `event_free_agents` PK can't represent "free agent for multiple divisions"

The table was created in
[20260514000200_event_free_agents.sql#L8-L14](../../supabase/migrations/20260514000200_event_free_agents.sql#L8-L14)
with `primary key (event_id, user_id)`. Bundle 5 added a NOT NULL
`division_id` column on top of that PK
([20260606000000_team_registration_model.sql#L271](../../supabase/migrations/20260606000000_team_registration_model.sql#L271)),
but the PK was never relaxed, so a single user can only be in **one**
division's free-agent pool per event. On a multi-division tournament
(AA + BB + Coed), a player who would happily fill in for any
division is forced to pick one — and the captain-claim flow then
only sees them in that bucket.

**Fix:** migration that drops the existing PK and reinstates it as
`(event_id, user_id, division_id)`. The free-agent UI then becomes
multi-select division on signup. Domain side: change
`JoinEventAsFreeAgentCommand.divisionId: DivisionId` to
`divisionIds: ReadonlyArray<DivisionId>` (or call the handler N times
inside the action — simpler, no aggregate change). Repository:
`attachFreeAgentToDivision` already supplies an explicit division id
per row, so the loop is straightforward. Backfill: existing rows are
already unique under `(event_id, user_id, division_id)` because the
old PK forced exactly one row per user-per-event — the wider PK is
strictly more permissive.

Files:
[packages/application/src/commands/join-event.handler.ts](../../packages/application/src/commands/join-event.handler.ts),
[packages/infrastructure/src/supabase-event-repository.ts](../../packages/infrastructure/src/supabase-event-repository.ts),
[apps/web/src/app/events/[id]/\_components/free-agent-signup-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/free-agent-signup-panel.tsx).

#### R2 (P1) — Host can't disable free-agent enrollment

The free-agent panel renders on every published tournament; the only
gate is `event.type === 'tournament'`
([free-agent-signup-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/free-agent-signup-panel.tsx))
and the RLS policy
([20260514000200_event_free_agents.sql#L30-L40](../../supabase/migrations/20260514000200_event_free_agents.sql#L30-L40)).
There is no host-configurable toggle — neither per-event nor
per-division — to opt out of free agents. Hosts running pure
captain-assembled tournaments today live with an empty free-agent
section and field "what is this for?" questions.

**Fix:** add `event_divisions.allow_free_agents boolean not null
default true`. Surface as a checkbox in the new-/edit-event division
repeater. Domain: `Division.allowFreeAgents` flag + invariant in
`VolleyballEvent.joinAsFreeAgent` (throw `InvariantViolation` when
the chosen division has it disabled). UI: hide
`FreeAgentSignupPanel` from `TournamentRegisterPanel` (and set
`teamEnabled=true` only) when no division accepts free agents; when
some divisions accept them, the existing single/multi-division
`<select>` filters to the eligible set. Reads: extend
`DivisionLite` projection in
[packages/domain/src/events/division.ts](../../packages/domain/src/events/division.ts).

Files: same set as R1 plus
[apps/web/src/app/events/new/new-event-form.tsx](../../apps/web/src/app/events/new/new-event-form.tsx)
(DivisionsRepeater) and
[apps/web/src/app/events/[id]/edit/edit-event-form.tsx](../../apps/web/src/app/events/%5Bid%5D/edit/edit-event-form.tsx).

#### R3 (P2) — `team_registration_mode` is event-level; user asked for per-division

> **Closed by Bundle 119 (2026-05-27).** ADR
> [0016](../adr/0016-per-division-team-registration-mode.md) supersedes
> ADR 0008 §2 and amends ADR 0012's matrix to per-division. Migration
> [20260711000000_per_division_team_registration_mode.sql](../../supabase/migrations/20260711000000_per_division_team_registration_mode.sql)
> moves the column, backfills from the parent, drops
> `events.team_registration_mode` in the same file, and rewrites the
> two team-insert RLS policies to join `event_divisions`. Domain
> `assertRegistrationConfigValid()` now iterates per division. UI in
> [event-signup-area.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/event-signup-area.tsx)
> renders both ad-hoc and roster sub-panels stacked when an event
> mixes modes. See [Bundle 119 journal](../journal/2026-05-digest.md#bundle-119).
> Original analysis kept below for context.

[ADR 0008 §2](../adr/0008-team-registration-paradigm.md) explicitly
chose **one mode per event** ("the simplest thing that fits 95% of
tournaments"). User's re-audit ask reopens the door:
multi-division tournaments where the AA division wants rostered
school teams and BB wants ad-hoc rec teams currently can't model
that — host has to pick one or split into two events.

**Fix path A (recommended if ADR is reopened):** move
`team_registration_mode` from `events` to `event_divisions`. Backfill
each row from the parent event's value. Domain: drop
`VolleyballEvent.teamRegistrationMode`, add
`Division.teamRegistrationMode`. UI:
`TournamentRegisterPanel` reads the picked division's mode after
the division choice, not before.

**Fix path B (if ADR stays):** close as Wontfix; document that
"divisions of an event share a registration mode" is intentional.
Either way, write an ADR amendment so the next reader doesn't
re-discover the gap.

**Dependency:** R3 is the right unlock for the deferred
"division → mode → roster → pay" wizard from ADR 0008 §6 — the
wizard is awkward today because step 2 (mode) is constant across
all divisions. Pursuing R3 first makes the wizard worth shipping.

#### R4 (P2) — Walk-ins are not a first-class registration type ✅ Closed (Bundle 120)

**Resolution:** Shipped 2026-05-27 via ADR
[0017](../adr/0017-walk-in-registrations.md) and migration
[20260712000000_walk_in_registrations.sql](../../supabase/migrations/20260712000000_walk_in_registrations.sql).
Walk-ins are now first-class: `event_team_registrations.source` is the
discriminator, `captain_id` is nullable, and walk-in captains carry
freeform `captain_display_name` + `captain_phone`. Payment goes
through a new `markPaidCash` domain method that records a freeform
`payment_note` (cap 500 chars). The host gets an "Add walk-in team"
form (ad-hoc divisions only) and a "Mark paid (cash)" action on
walk-in rows in [host-ad-hoc-teams-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/host-ad-hoc-teams-panel.tsx);
the public roster renders a violet "Walk-in" pill in
[teams-registered-section.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/teams-registered-section.tsx).
Product question on identity resolved as "nullable `captain_id` +
freeform name + phone" — walk-ins are host-created for the life of
the event and never become captain-replaceable. See journal entry
[docs/journal/2026-05-digest.md#bundle-120](../journal/2026-05-digest.md#bundle-120).

<details>
<summary>Original finding (kept for context)</summary>

User listed "rostered team / ad-hoc team / walk-in" as three peers.
Currently:

- **Captain-created ad-hoc:** captain self-registers via
  `AdHocTeamSignupPanel`.
- **Host-created ad-hoc:** host registers a team on behalf of a
  captain via
  [host-team-registration-actions.ts](../../apps/web/src/app/events/%5Bid%5D/host-team-registration-actions.ts).
  This is the closest thing to a "walk-in" today but is implemented
  as ordinary ad-hoc with the host as proxy.

Gaps if walk-in is meant to be distinct:

- No separate payment path (`event_team_payments` assumes a captain
  with a Stripe Checkout session; walk-ins typically pay cash at
  the door).
- No badge / pill on `TeamsRegisteredSection` to distinguish
  walk-ins so attendees can tell which slots are still in flux.
- No captain account required → the existing
  `event_team_registrations.captain_id` FK to `profiles(id)` makes
  this hard. Walk-ins would need either a synthetic profile (anon
  auth — already supported) or a nullable `captain_id` + freeform
  `captain_display_name`.

**Fix sketch:** add `event_team_registrations.source` enum
(`captain` | `host` | `walk_in`) with a default of `captain`. Host
panel sets `host` on creation; new "Walk-in" host action sets
`walk_in` and skips the captain-profile lookup (uses the host's
profile as the operator id, captures captain name + phone as
freeform text). `TeamsRegisteredSection` renders a "Walk-in" pill
beside the payment pill. Payment: walk-ins start at
`payment_status='none'` and the host marks them paid via a new
"Mark paid (cash)" host action (no Stripe path).

**Product question** to settle before code lands: should walk-ins
have the same captain-replaceable identity as ad-hoc, or are they
truly host-owned for the life of the event? Answer determines
whether `captain_id` becomes nullable.

</details>

#### R6 (P3 — architectural) — `event_id` on team-registration tables is redundant with `division_id`

`event_teams.event_id` and `event_team_registrations.event_id` both
co-exist with `division_id`, but `event_divisions.event_id` already
determines the parent event uniquely. The double-FK encodes the
older "event owns teams" model — and creates a constraint footgun:
nothing enforces that
`event_teams.division_id ∈ event_divisions where event_id = event_teams.event_id`
(an admin write could attach a team to division X under event A
while claiming event B). Today the application layer always pairs
the writes so it doesn't bite, but the schema can't catch a
regression.

**Fix (additive):** add a check constraint or trigger that asserts
`event_id` matches the division's parent. Long-term, drop the
column entirely and derive it via the division join in reads
(touches every read path — large diff, low payoff; defer).

**Note:** the user's "event doesn't need teams associated to it"
ask is largely already true at the **read** layer — pages source
teams from the `event.teams` projection, which is built from
`event_teams` filtered by event_id. Moving to "build the projection
by walking divisions" is the natural next step and falls out of
this cleanup.

### UX

#### R5 (P2) — Single-division tournaments still render the full Divisions card

[`DivisionsSection`](../../apps/web/src/app/events/%5Bid%5D/_components/divisions-section.tsx)
renders unconditionally when `divisions.length > 0`. On a
single-division tournament the card duplicates skill tier, format,
gender, age group, team composition, capacity, and price — all of
which `EventHero` already shows. The page reads:

> **Outdoor Open AA — Sat May 30 · Newport News**
> Tier · Outdoor · Sixes · Open · Adult · 8 spots · $25 / team
>
> **Divisions (1)**
>
> > Outdoor Open AA
...
**Fix:** in
[page.tsx](../../apps/web/src/app/events/%5Bid%5D/page.tsx), only
render `<DivisionsSection>` when `event.divisions.length > 1`.
Single-division metadata is already covered by `EventHero` (which
already takes `divisionCount` and the first division's `skillTier`
/ `tierLabel`). The host-tools `<HostDivisionsManager>` should
still render for hosts so they can split into multiple divisions.

Knock-on cleanups for single-division mode (cheap once R5 lands):

- `TeamsRegisteredSection`: drop the "Division · " prefix from
  ad-hoc rows when `divisions.length === 1` (the divisions prop is
  already plumbed through).
- `FreeAgentSignupPanel`: the section header could read "Free
  agents" instead of grouping into one trivial subsection.
- `TournamentRegisterPanel`: subline "8 teams · 3 free agents"
  stays useful; no change.

### Coverage notes / not-findings

- The Bundle 68 closure of Model P1 (`division_id` NOT NULL) holds
  up — verified the constraint is live and routed through the
  `attach*ToDivision` ports. No new gap.
- Bundle 62 closure of "ad-hoc live-vs-dead" holds up — re-traced
  all five server actions + signup panel + host panel + Stripe
  branch and they're all wired.
- The `event_teams` PK `(event_id, team_id)` from
  [20260512000000_init.sql#L132-L136](../../supabase/migrations/20260512000000_init.sql#L132-L136)
  prevents the same persistent `teams` row from registering for
  two divisions of one event. In practice the same school's "Varsity"
  and "JV" are separate `teams` rows, so this hasn't surfaced as a
  pain — flagging here only for completeness, not as an open
  finding.
- The Bundle 105 promotion migration
  ...
  means every ad-hoc registration now has a backing `teams` row +
  `event_teams` link. That keeps bracket seeding working but
  reinforces the "event owns teams" shape R6 is pushing back on.
  Worth re-examining if R3/R4 land and the team taxonomy shifts.
...
| 2026-05-22 | Payment policy P1 — `(team-led) + (per_player) + (!payments_off_platform)` silently accepted at create/edit                                                                                                                | Added shared boundary validator `validateTeamPricing`; wired into `createEventAction` and `editEventAction`. Returns an actionable error naming the three ADR 0007 §3 resolutions (switch division to per-team, disable team mode, or set off-platform). See [Bundle 3 journal](../journal/2026-05-digest.md#bundle-3).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | [apps/web/src/lib/event-team-pricing-validation.ts](../../apps/web/src/lib/event-team-pricing-validation.ts), [apps/web/src/app/events/new/actions.ts](../../apps/web/src/app/events/new/actions.ts), [apps/web/src/app/events/%5Bid%5D/edit/actions.ts](../../apps/web/src/app/events/%5Bid%5D/edit/actions.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| 2026-05-23 | Model P1 — Roster-mode per-team captain checkout missing (captain registered through `event_teams` with no Stripe path)                                                                                                    | Added sidecar `event_team_payments` table + `EventTeamPayment` aggregate (state machine mirrors `EventTeamRegistration`) + repository port + Supabase adapter + `startRosterTeamCheckout` server action + Stripe webhook branches (paid/expired/refund) for `kind='roster_team_payment'` + parallel success/cancel routes. `TournamentSignupPanel` now surfaces a `Pay — $X` / `Resume checkout` button inline with the captain's registered team and status pills for all viewers. Closes the last open P1 on this audit. See [Bundle 4 journal](../journal/2026-05-digest.md#bundle-4).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | [supabase/migrations/20260608000000_event_team_payments.sql](../../supabase/migrations/20260608000000_event_team_payments.sql), [packages/domain/src/events/event-team-payment.ts](../../packages/domain/src/events/event-team-payment.ts), [packages/infrastructure/src/supabase-event-team-payment-repository.ts](../../packages/infrastructure/src/supabase-event-team-payment-repository.ts), [apps/web/src/app/events/%5Bid%5D/roster-team-checkout-actions.ts](../../apps/web/src/app/events/%5Bid%5D/roster-team-checkout-actions.ts), [apps/web/src/app/api/webhooks/stripe/route.ts](../../apps/web/src/app/api/webhooks/stripe/route.ts), [apps/web/src/app/events/%5Bid%5D/\_components/tournament-signup-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/tournament-signup-panel.tsx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2026-05-24 | Recommended sequencing step 1 — team-paradigm ADR not written                                                                                                                                                              | Wrote [ADR 0008](../adr/0008-team-registration-paradigm.md) ratifying per-event single-mode (ad-hoc default for tournaments), two-aggregate split (`Team` vs `EventTeamRegistration`), sidecar payment (`EventTeamPayment` for roster, inline column for ad-hoc), side-step `attach*ToDivision` ports as a documented doctrine, captain-pre-pay deferred, and single-flow collapse as a pure UX bundle. ADR index updated to include 0007 + 0008. Discovery confirmed ad-hoc scaffolding is live end-to-end (handlers, signup panel, host panel, division-winner picker) — Model P2 ad-hoc audit no longer needs a separate dead-code pass. See [Bundle 6 journal](../journal/2026-05-digest.md#bundle-6).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | [docs/adr/0008-team-registration-paradigm.md](../adr/0008-team-registration-paradigm.md), [docs/adr/README.md](../adr/README.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 2026-05-24 | UX P2 — Tournament signup was two tabs that should be one flow + stale UX P2 for `PaidTicketPanel` dual CTA                                                                                                                | Replaced `TournamentRegistrationTabs` (`role=tablist`) with `TournamentRegisterPanel` — a single `'use client'` wrapper that renders a `role=radiogroup` "How are you signing up?" picker above the existing team and free-agent server-component panels (passed in as `ReactNode` children to preserve the server-action boundary). Hides the team branch entirely when `team_registration_mode === null`; defaults to the free-agent panel on free-agent-only events. Also verified the `PaidTicketPanel` dual-CTA finding is stale — the panel already gates on `paymentsOffPlatform` to show exactly one CTA. Full division→mode→roster→pay wizard (ADR 0008 §6) deferred. See [Bundle 7 journal](../journal/2026-05-digest.md#bundle-7).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | [apps/web/src/app/events/%5Bid%5D/\_components/tournament-register-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/tournament-register-panel.tsx), [apps/web/src/app/events/%5Bid%5D/page.tsx](../../apps/web/src/app/events/%5Bid%5D/page.tsx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2026-05-24 | UX P1 (partial) + Model P2 — Free-agent signup had no division picker and `event_free_agents.division_id` was never populated                                                                                              | `JoinEventAsFreeAgentCommand` now takes a required `divisionId`; handler validates the division belongs to the event then calls new `attachFreeAgentToDivision` port (mirrors `attachTeamToDivision`). `FreeAgentSignupPanel` renders a hidden input on single-division events and a required `<select>` on multi-division, and shows a division pill on each free-agent row. Detail read model + Supabase select extended to surface `divisionId`. No migration needed (column added in 20260605000100). See [Bundle 5 journal](../journal/2026-05-digest.md#bundle-5).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | [packages/domain/src/events/event-repository.ts](../../packages/domain/src/events/event-repository.ts), [packages/application/src/commands/join-event.handler.ts](../../packages/application/src/commands/join-event.handler.ts), [packages/infrastructure/src/supabase-event-repository.ts](../../packages/infrastructure/src/supabase-event-repository.ts), [apps/web/src/app/events/%5Bid%5D/free-agent-actions.ts](../../apps/web/src/app/events/%5Bid%5D/free-agent-actions.ts), [apps/web/src/app/events/%5Bid%5D/\_components/free-agent-signup-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/free-agent-signup-panel.tsx), [apps/web/src/app/events/%5Bid%5D/page.tsx](../../apps/web/src/app/events/%5Bid%5D/page.tsx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2026-05-23 | Model P3 — team format vs division format misclassified as `UnauthorizedError`                                                                                                                                             | Reclassified both the cross-event and cross-division format-mismatch throws in `RegisterTeamHandler` from `UnauthorizedError` to `ValidationError`, matching the documented intent from the 2026-05-22 remediation log row and routing the user to the wired-up `?team=invalid` → "Team format doesn't match the event." flash instead of the misleading `?team=forbidden` → "Only the team captain can do that." See [Bundle 52 journal](../journal/2026-05-digest.md#bundle-52).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | [packages/application/src/commands/team.handler.ts](../../packages/application/src/commands/team.handler.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 2026-05-23 | Test coverage follow-up — `RegisterTeamHandler` had no unit tests after Bundle 52 reclassification                                                                                                                         | Added [team.handler.test.ts](../../packages/application/src/commands/team.handler.test.ts) (7 cases) with in-memory `TeamRepository` + `EventRepository` doubles. Covers: happy path (`attachTeamToDivision` invoked, aggregate not `save()`d — preserves the Bundle 52 / Bundle 2 doctrine that the division id lives only on the join row); `NotFoundError` for missing team / event / division; `UnauthorizedError` for non-captain requester; and **`ValidationError` for both cross-event and cross-division format mismatch** — locks in the Bundle 52 reclassification so future refactors can't quietly revert to `UnauthorizedError`. See [Bundle 55 journal](../journal/2026-05-digest.md#bundle-55).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | [packages/application/src/commands/team.handler.test.ts](../../packages/application/src/commands/team.handler.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2026-05-23 | Model P2 — Ad-hoc team registration: live vs. dead-code audit (open since Bundle 6 discovery note)                                                                                                                         | Re-traced the ad-hoc path end-to-end and formally closed as ✅ Live (verified). Every layer wired and reachable: schema (migration `20260606000000`), `team_registration_mode` selector in both new + edit event forms, detail-loader hydration of public + host rows when mode is `ad_hoc`, five server actions (register / rename / add-member / remove-member / withdraw) in `ad-hoc-team-actions.ts`, captain `AdHocTeamSignupPanel` slot inside `TournamentRegisterPanel`, host `HostAdHocTeamsPanel` inside the `Host tools` `<details>`, captain Stripe path via `team-checkout-actions.ts` branching on `TeamRegistrationMode.AdHoc`, post-event division-winner picker handling both FKs. No dead code surfaced; finding moves from "confirm" to "closed." See [Bundle 62 journal](../journal/2026-05-digest.md#bundle-62).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | [apps/web/src/app/events/%5Bid%5D/\_components/event-signup-area.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/event-signup-area.tsx), [apps/web/src/app/events/%5Bid%5D/\_components/host-tools-section.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/host-tools-section.tsx), [apps/web/src/app/events/%5Bid%5D/ad-hoc-team-actions.ts](../../apps/web/src/app/events/%5Bid%5D/ad-hoc-team-actions.ts), [apps/web/src/app/events/%5Bid%5D/\_loaders/load-event-detail.ts](../../apps/web/src/app/events/%5Bid%5D/_loaders/load-event-detail.ts), [apps/web/src/app/events/%5Bid%5D/edit/edit-event-form.tsx](../../apps/web/src/app/events/%5Bid%5D/edit/edit-event-form.tsx), [supabase/migrations/20260606000000_team_registration_model.sql](../../supabase/migrations/20260606000000_team_registration_model.sql)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 2026-05-23 | UX P3 — Free-agent list has no division grouping (partial close of "no division context")                                                                                                                                  | Replaced the flat free-agent `<ul>` with one section per division on multi-division events. Sections render in event-divisions order with header + count + empty state ("No free agents in this division yet."); legacy null-`division_id` rows fall into a trailing **Unassigned** bucket. Single-division events keep the flat list (no behaviour change). Added `groupFreeAgentsByDivision` helper + extracted `FreeAgentRow` co-located in the same file. Per-row division pill dropped inside grouped mode (now redundant with the section header). Captain-claim affordance — the other half of the original UX P3 — is still open. See [Bundle 63 journal](../journal/2026-05-digest.md#bundle-63).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | [apps/web/src/app/events/%5Bid%5D/\_components/free-agent-signup-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/free-agent-signup-panel.tsx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 2026-05-23 | Model P1 — `event_attendees.division_id` nullable (and parallel `event_teams` / `event_free_agents` columns) — schema enforcement bookkeeping                                                                              | Re-traced both layers and formally closed as ✅ Live (verified, schema-enforced). The schema NOT NULL constraint landed in migration `20260606000000_team_registration_model.sql` (lines 269–271 on the three tables, preceded by a backfill in sections 5a/5b/5c) but the inline finding header was never flipped. Multi-division writes route through `attachTeamToDivision` (Bundle 1) / `attachFreeAgentToDivision` (Bundle 5), supplying `division_id` explicitly; single-division writes are filled by the `fill_default_division_id` trigger. The open-play `event_attendees` case never hits multi-division because the create form gates `DivisionsRepeater` on `EventType.Tournament` ([new-event-form.tsx#L515-L565](../../apps/web/src/app/events/new/new-event-form.tsx#L515)), which also discharges the open-play half of UX P1's "no division picker" finding under the same rationale. No code changes. See [Bundle 68 journal](../journal/2026-05-digest.md#bundle-68).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | [supabase/migrations/20260606000000_team_registration_model.sql](../../supabase/migrations/20260606000000_team_registration_model.sql#L189-L271), [packages/infrastructure/src/supabase-event-repository.ts](../../packages/infrastructure/src/supabase-event-repository.ts#L1334-L1362), [apps/web/src/app/events/new/new-event-form.tsx](../../apps/web/src/app/events/new/new-event-form.tsx#L515)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-05-27 | Model R1 (P1) + Model R2 (P1) + Model R6 (P3) — Free agents pinned to one division per event; no host opt-out for FA enrollment; cross-aggregate `event_id` / `division_id` could disagree                                 | Single migration `20260710000000_division_centric_registration.sql` does three things: (R1) widens `event_free_agents` PK from `(event_id, user_id)` to `(event_id, user_id, division_id)` so a player can free-agent into multiple divisions of the same event; (R2) adds `event_divisions.allow_free_agents boolean not null default true` + rebuilds the `event_free_agents_insert` RLS policy to require `allow_free_agents = true` on the target division; (R6) installs `assert_division_event_consistency()` trigger on `event_teams`, `event_team_registrations`, `event_free_agents`, `event_attendees` to guarantee `(event_id, division_id)` references the same parent event. Domain: `Division` gained `allowFreeAgents` (default true via factory); `VolleyballEvent.joinAsFreeAgent` now validates the division is on the event + accepts free agents (throws `NotFoundError('division')` or `InvariantViolation`). App handler `JoinEventAsFreeAgentHandler` defers the precheck to the aggregate. Infra repo + `DivisionLite` thread the new field. UI: host divisions manager + create-event repeater both render an "Accept free-agent signups" checkbox (defaults checked); `FreeAgentSignupPanel` hides the form when no division accepts FAs and maps the new `InvariantViolation` to a `?fa=fa_disabled` flash; `TournamentRegisterPanel` collapses the picker entirely when `freeAgentEnabled=false`. Test sweep updates the free-agent describe block to construct a per-team-priced opt-in division and adds two new cases (opt-out → `InvariantViolation`, unknown division → `NotFoundError`). R3 (per-division `team_registration_mode`) + R4 (walk-ins) + R5 (single-division UI collapse) are tracked separately. See [Bundle 118 journal](../journal/2026-05-digest.md#bundle-118). | [supabase/migrations/20260710000000_division_centric_registration.sql](../../supabase/migrations/20260710000000_division_centric_registration.sql), [packages/domain/src/events/division.ts](../../packages/domain/src/events/division.ts), [packages/domain/src/events/volleyball-event.ts](../../packages/domain/src/events/volleyball-event.ts), [packages/application/src/commands/join-event.handler.ts](../../packages/application/src/commands/join-event.handler.ts), [packages/infrastructure/src/supabase-event-repository.ts](../../packages/infrastructure/src/supabase-event-repository.ts), [packages/types/src/events.ts](../../packages/types/src/events.ts), [apps/web/src/app/events/[id]/\_components/free-agent-signup-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/free-agent-signup-panel.tsx), [apps/web/src/app/events/[id]/\_components/tournament-register-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/tournament-register-panel.tsx), [apps/web/src/app/events/[id]/\_components/host-divisions-manager.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/host-divisions-manager.tsx), [apps/web/src/app/events/[id]/division-actions.ts](../../apps/web/src/app/events/%5Bid%5D/division-actions.ts), [apps/web/src/app/events/[id]/free-agent-actions.ts](../../apps/web/src/app/events/%5Bid%5D/free-agent-actions.ts), [apps/web/src/app/events/new/actions.ts](../../apps/web/src/app/events/new/actions.ts), [apps/web/src/app/events/new/\_components/divisions-repeater.tsx](../../apps/web/src/app/events/new/_components/divisions-repeater.tsx)                                                                                                                                                                                                                                                                                                                                                                                |
| 2026-05-27 | Model R3 (P2) — `team_registration_mode` was event-level; user asked for per-division (multi-division tournaments couldn't run A=roster + BB=ad-hoc on one event)                                                          | Wrote **[ADR 0016](../adr/0016-per-division-team-registration-mode.md)** superseding ADR 0008 §2 and amending ADR 0012's matrix. Single migration [20260711000000_per_division_team_registration_mode.sql](../../supabase/migrations/20260711000000_per_division_team_registration_mode.sql) adds nullable `event_divisions.team_registration_mode`, backfills from `events.team_registration_mode`, rewrites `event_team_registrations_insert` + `event_team_payments_insert` RLS policies to join `event_divisions` (and `event_teams` for the payment policy), drops `events.team_registration_mode`, and rebuilds `events_view`. Domain: `Division` gained the field; `VolleyballEvent.assertRegistrationConfigValid()` now iterates per division (ad-hoc/roster → non-solo + per-team; null → solo + per-player). App layer: `CreateEventHandler` + `UpdateEventDivisionHandler` thread the new per-division field; `EventTeamRegistrationHandler` + both checkout actions look up the target division first and gate on its mode. UI: per-row `<select>` added to `DivisionsRepeater` + `HostDivisionsManager`; event-level `<select>` removed from new + edit forms; `EventSignupArea` now renders both `AdHocTeamSignupPanel` and `TournamentSignupPanel` stacked when an event mixes modes. Default for new tournament + non-solo divisions remains `ad_hoc` (preserved from prior event-level default). Test sweep updates the matrix tests to assert per-division behaviour. See [Bundle 119 journal](../journal/2026-05-digest.md#bundle-119).                                                                                                                                                                                                                                                          | [docs/adr/0016-per-division-team-registration-mode.md](../adr/0016-per-division-team-registration-mode.md), [supabase/migrations/20260711000000_per_division_team_registration_mode.sql](../../supabase/migrations/20260711000000_per_division_team_registration_mode.sql), [packages/domain/src/events/division.ts](../../packages/domain/src/events/division.ts), [packages/domain/src/events/volleyball-event.ts](../../packages/domain/src/events/volleyball-event.ts), [packages/application/src/commands/create-event.handler.ts](../../packages/application/src/commands/create-event.handler.ts), [packages/application/src/commands/event-division.handler.ts](../../packages/application/src/commands/event-division.handler.ts), [packages/application/src/commands/event-team-registration.handler.ts](../../packages/application/src/commands/event-team-registration.handler.ts), [packages/infrastructure/src/supabase-event-repository.ts](../../packages/infrastructure/src/supabase-event-repository.ts), [packages/types/src/events.ts](../../packages/types/src/events.ts), [apps/web/src/app/events/new/actions.ts](../../apps/web/src/app/events/new/actions.ts), [apps/web/src/app/events/new/\_components/divisions-repeater.tsx](../../apps/web/src/app/events/new/_components/divisions-repeater.tsx), [apps/web/src/app/events/[id]/edit/actions.ts](../../apps/web/src/app/events/%5Bid%5D/edit/actions.ts), [apps/web/src/app/events/[id]/\_components/host-divisions-manager.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/host-divisions-manager.tsx), [apps/web/src/app/events/[id]/\_components/event-signup-area.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/event-signup-area.tsx), [apps/web/src/app/events/[id]/team-checkout-actions.ts](../../apps/web/src/app/events/%5Bid%5D/team-checkout-actions.ts), [apps/web/src/app/events/[id]/roster-team-checkout-actions.ts](../../apps/web/src/app/events/%5Bid%5D/roster-team-checkout-actions.ts) |
| 2026-05-27 | Model R4 (P3) — Walk-in team registrations were not first-class (no account-free captain, no cash-payment path, no UI distinction)                                                                                         | Wrote **[ADR 0017](../adr/0017-walk-in-registrations.md)** amending ADRs 0007 / 0008 / 0016. Migration [20260712000000_walk_in_registrations.sql](../../supabase/migrations/20260712000000_walk_in_registrations.sql) adds `source text` (default `'captain'`, check in `captain/host/walk_in`), makes `captain_id` nullable, adds `captain_display_name` (1–80) + `captain_phone` (1–40) + `payment_note` (≤500), installs a discriminant constraint + source index, and rewrites `event_team_registrations_insert` RLS with two branches (captain self-signup OR host insert on published ad-hoc divisions when `source ∈ {host, walk_in}`). Domain: `EventTeamRegistration` gained `RegistrationSource` enum, nullable `captainId`, `assertSourceIdentity` guard, and a new `markPaidCash({amountCents, paidAt, note})` state transition. Application: `RegisterWalkInTeamCommand` + `MarkWalkInPaidCashCommand` + matching handlers (walk-ins gated on `division.teamRegistrationMode === AdHoc`). Infrastructure mapper round-trips the new columns; `ensureBackingTeam` falls back to `events.host_id` as the `teams.captain_id` for walk-ins so the bracket reader and `events_view.team_count` keep working. UI: `HostAdHocTeamsPanel` grows an "Add walk-in team" collapsed form + a "Mark paid (cash)" form per walk-in row; `TeamsRegisteredSection` + the host panel both render a violet "Walk-in" pill. Closes the last open Bundle 117 re-audit finding. See [Bundle 120 journal](../journal/2026-05-digest.md#bundle-120).                                                                                                                                                                                                                                                                          | [docs/adr/0017-walk-in-registrations.md](../adr/0017-walk-in-registrations.md), [supabase/migrations/20260712000000_walk_in_registrations.sql](../../supabase/migrations/20260712000000_walk_in_registrations.sql), [packages/domain/src/events/event-team-registration.ts](../../packages/domain/src/events/event-team-registration.ts), [packages/application/src/messages.ts](../../packages/application/src/messages.ts), [packages/application/src/commands/event-team-registration.handler.ts](../../packages/application/src/commands/event-team-registration.handler.ts), [packages/infrastructure/src/supabase-event-team-registration-repository.ts](../../packages/infrastructure/src/supabase-event-team-registration-repository.ts), [apps/web/src/lib/handlers.ts](../../apps/web/src/lib/handlers.ts), [apps/web/src/app/events/[id]/walk-in-team-actions.ts](../../apps/web/src/app/events/%5Bid%5D/walk-in-team-actions.ts), [apps/web/src/app/events/[id]/\_loaders/load-event-detail.ts](../../apps/web/src/app/events/%5Bid%5D/_loaders/load-event-detail.ts), [apps/web/src/app/events/[id]/\_components/host-ad-hoc-teams-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/host-ad-hoc-teams-panel.tsx), [apps/web/src/app/events/[id]/\_components/host-tools-section.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/host-tools-section.tsx), [apps/web/src/app/events/[id]/\_components/teams-registered-section.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/teams-registered-section.tsx), [apps/web/src/app/events/[id]/\_components/ad-hoc-team-signup-panel.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/ad-hoc-team-signup-panel.tsx)                                                                                                                                                                                                                                                                                               |
| 2026-05-27 | Bundle 121 — Payment policy P1 (2026-05-22) was over-applied: free divisions (`price_cents = 0`) rejected for `(team-led + per_player)` even though no payment is routed (host's question "why can't a division be free?") | Relaxed both the domain rule (`VolleyballEvent.assertRegistrationConfigValid`) and the boundary mirror (`validateTeamPricing`): Rules 2 & 3's price-unit clauses now skip when `(d.priceCents ?? 0) <= 0`. Composition halves of both rules and Rule 4 (off-platform doesn't relax anything) preserved for paid divisions. Write boundary normalizes the persisted `price_unit` per resolved `team_registration_mode` when free (team-led → `per_team`, individual → `per_player`) so stored rows stay coherent the moment the host adds a price. UI hides the "Charge" select on free divisions in both `DivisionsRepeater` (create form) and `HostDivisionsManager` (edit) — picker only appears once `priceUsd > 0`. `HostDivisionsManager` promoted `priceUsd` from `defaultValue` to controlled state so the gate is reactive. Test sweep flips the prior rejection case to acceptance and adds an off-platform variant. ADR 0012 amended with a "Free-division exemption" section. See [Bundle 121 journal](../journal/2026-05-digest.md#bundle-121).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | [docs/adr/0012-registration-paradigm-invariants.md](../adr/0012-registration-paradigm-invariants.md), [packages/domain/src/events/volleyball-event.ts](../../packages/domain/src/events/volleyball-event.ts), [apps/web/src/lib/event-team-pricing-validation.ts](../../apps/web/src/lib/event-team-pricing-validation.ts), [apps/web/src/app/events/new/actions.ts](../../apps/web/src/app/events/new/actions.ts), [apps/web/src/app/events/[id]/division-actions.ts](../../apps/web/src/app/events/%5Bid%5D/division-actions.ts), [apps/web/src/app/events/new/\_components/divisions-repeater.tsx](../../apps/web/src/app/events/new/_components/divisions-repeater.tsx), [apps/web/src/app/events/[id]/\_components/host-divisions-manager.tsx](../../apps/web/src/app/events/%5Bid%5D/_components/host-divisions-manager.tsx), [packages/domain/src/events/volleyball-event.test.ts](../../packages/domain/src/events/volleyball-event.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- ~~**Model R1 (P1, new 2026-05-27)** — `event_free_agents` PK
  `(event_id, user_id)` blocks a player from joining multiple
  divisions' free-agent pools per event.~~ — **closed** by Bundle 118
  ...
  widens the PK to `(event_id, user_id, division_id)`. App-side UI
  for picking _multiple_ divisions in one signup is deferred — the
  current panel still posts one division at a time, but the schema no
  longer rejects a second signup.
- ~~**Model R2 (P1, new 2026-05-27)** — No host toggle for free-agent
  enrollment.~~ — **closed** by Bundle 118 (2026-05-27): new
  `event_divisions.allow_free_agents` column (defaults `true`), domain
  invariant in `VolleyballEvent.joinAsFreeAgent`, RLS gate on
  `event_free_agents_insert`, host checkbox on both the create-event
  divisions repeater and the per-event divisions manager, and
  panel-level gating that hides the free-agent form (and collapses
  the picker in `TournamentRegisterPanel`) when no division accepts FAs.
- **Model R3 (P2, new 2026-05-27)** — ~~`team_registration_mode` is
  event-level; user asked for per-division.~~ — **closed** by Bundle 119
  (2026-05-27): ADR
  [0016](../adr/0016-per-division-team-registration-mode.md) +
  migration
  [20260711000000_per_division_team_registration_mode.sql](../../supabase/migrations/20260711000000_per_division_team_registration_mode.sql)
  move the column to `event_divisions`, backfill, drop the parent
  column, and rewrite both team-insert RLS policies to join
  `event_divisions`. Mixed-mode events render both `AdHocTeamSignupPanel`
  and `TournamentSignupPanel` stacked.
- **Model R4 (P3, new 2026-05-27)** — ~~Walk-ins are not a first-class
  registration type.~~ — **closed** by Bundle 120 (2026-05-27): ADR
  [0017](../adr/0017-walk-in-registrations.md) introduces the `source`
  discriminator, nullable `captain_id` with freeform
  `captain_display_name` + `captain_phone`, `markPaidCash` domain
  method with `payment_note`, and host UI ("Add walk-in team" form +
  "Mark paid (cash)" action + "Walk-in" pill). Migration
  [20260712000000_walk_in_registrations.sql](../../supabase/migrations/20260712000000_walk_in_registrations.sql)
  adds the columns, discriminant constraint, source index, and
  rewrites the insert RLS policy to allow host inserts on walk-ins
  for published ad-hoc divisions.
- ~~**Model R6 (P3, new 2026-05-27, architectural)** —
  `event_teams.event_id` / `event_team_registrations.event_id` are
  redundant with `division_id`.~~ — **closed** by Bundle 118 (2026-05-27):
  Bundle 118 migration installs `assert_division_event_consistency()`
  BEFORE INSERT/UPDATE triggers on `event_teams`,
  `event_team_registrations`, `event_free_agents`, and
  `event_attendees`, raising on cross-event/division mismatch.
  Dropping the redundant `event_id` columns themselves is intentionally
  deferred — the trigger is a cheap belt-and-braces win that satisfies
  the audit's "no contradictions can be persisted" intent without a
  column-removal sweep.
- **UX R5 (P2, new 2026-05-27)** — Single-division tournaments
  duplicate `EventHero` data in `DivisionsSection`. See **R5**.
  Fix: render `DivisionsSection` only when
  `divisions.length > 1`; drop division-prefix labels in the
  ad-hoc team list and free-agent section on single-division
  events.
