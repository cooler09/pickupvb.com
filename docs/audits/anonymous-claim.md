...
> branches. **Still verify in Supabase:** whether "Confirm email" is enabled —
> that determines AC-2's real-world severity (see the finding) and whether the
> guest-confirm-link path is exercised at all. See remediation log below.
>
...
>
> Grades: **0 P1 · 4 P2 · 6 P3.** Nothing is exploitable or data-losing; the
> P2s are conversion-funnel and stranded-user issues worth a sprint. AC-2 is
> **config-dependent** — promote to P1 if "Confirm email" is enabled on the
> Supabase project (see the finding).
...
### AC-2 · Guest email-confirmation links use the wrong redirect — P2 (→ P1 if email-confirm is on)
...
**Severity caveat:** the stranding only fires when "Confirm email" is enabled.
Verify the toggle in the Supabase dashboard (dev `rjkqldsxeidqzsameker`, prod
`civomzrspifpoufajmmw`). If on → **P1**. If off, the email is set silently and
the claim flow's own confirmation handles it, so it's latent — still worth
fixing for the one-click-claim upside.
...
| **AC-2**  | New `buildClaimEmailRedirect(next?)` helper centralizes the `/auth/callback?next=/reset-password?from=claim` redirect; wired into the claim action **and** both guest email-attach paths (`updateUser({ email }, { emailRedirectTo })`), so any confirmation click lands on set-password instead of the default Site URL. Guest paths thread `next=/events/{id}`. ([server-redirects.ts](../../apps/web/src/lib/server-redirects.ts), [guest-actions.ts](../../apps/web/src/app/events/%5Bid%5D/guest-actions.ts), [checkout-actions.ts](../../apps/web/src/app/events/%5Bid%5D/checkout-actions.ts)) **Verify the "Confirm email" toggle** to confirm severity. |
