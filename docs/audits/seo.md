...
> **Status update (2026-06-12 — community-listing source association):**
> Enhancement bundle (not a re-audit) targeting two goals on the community
> surface: (1) keep the pages indexable/crawlable — already largely true
> (canonical, OG, sitemap inclusion of `active`/`claim_pending`, `SportsEvent`
>
> - `Breadcrumb` JSON-LD, tailored OG image) — and (2) **associate each listing
>   with the off-platform event it links out to** so PickupVB starts surfacing
>   for searches about those sources. The deliverable lever is schema.org
>   **`sameAs`** on the listing's `SportsEvent` (entity-identity → "same event as
>   that Facebook/Meetup/league page"); pairing it with `description` + `image`
>   and an `ItemList` on the `/community` index. The outbound CTA is now
>   `rel="ugc nofollow"` (correct UGC attribution) — link equity intentionally
>   **not** passed out of an unverified hyperlink; the crawlable association is
>   the structured `sameAs`, so the `/leaving` interstitial stays for untrusted
>   hosts. Honest scope note: this makes us **rank for** those events, not appear
>   **on** Facebook/Meetup (that needs inbound links from them). Quad-green. See
>   the **Remediation log** entry (2026-06-12) below.
>
...
> negligible upside). **Follow-up (same day):** migrated the last 8 raw inline
> `<script type="application/ld+json">` blocks — the 7 tool pages'
> `WebApplication` + `FAQPage` JSON-LD and the root layout's
> `Organization` + `WebSite` JSON-LD — onto the shared
> [`JsonLd`](../../apps/web/src/components/json-ld.tsx) emitter, so **every**
> structured-data blob on the site now routes through the one XSS-escaping
> path. These values are static/server-controlled (no live exploit), but the
> consolidation removes the divergent pattern the next author would otherwise
> copy. Verify quad green. See the **Remediation log** entries below.
...
| 2026-06-05 | Follow-up: last raw inline JSON-LD → shared `JsonLd`                    | Migrated the final 8 raw `<script type="application/ld+json">` + `JSON.stringify` blocks — the 7 tool pages' `WebApplication` + `FAQPage` JSON-LD and the root layout's `Organization` + `WebSite` JSON-LD — onto the shared `JsonLd` emitter. Every structured-data blob on the site now routes through the one XSS-escaping path. Static/server-controlled values (no live exploit); consolidation removes the divergent pattern.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | [layout.tsx](../../apps/web/src/app/layout.tsx), [tools/scoreboard/page.tsx](../../apps/web/src/app/tools/scoreboard/page.tsx), [team-randomizer](../../apps/web/src/app/tools/team-randomizer/page.tsx), [scheduler](../../apps/web/src/app/tools/scheduler/page.tsx), [seeding](../../apps/web/src/app/tools/seeding/page.tsx), [standings](../../apps/web/src/app/tools/standings/page.tsx), [rotation](../../apps/web/src/app/tools/rotation/page.tsx), [timer](../../apps/web/src/app/tools/timer/page.tsx)                                 |
| 2026-06-12 | Enhancement: community listing → external-source association (`sameAs`) | Enriched the community-listing `SportsEvent` JSON-LD so each indexed listing is tied to its off-platform source: added **`sameAs: [externalUrl]`** (the entity-identity signal that this page describes the _same_ event as the linked Facebook/Meetup/league page — the lever for surfacing in searches about that off-platform event), plus `description` (submitter text, capped 300, with a place-aware fallback) and `image` (the listing's `opengraph-image` route). Pinned the JSON-LD `url`/`image` base to `PROD_APP_URL` (drops the last hand-typed apex in this file). Marked the outbound CTA `rel="ugc"` (user-submitted link) on top of the existing `nofollow`; the crawlable source link is carried by `sameAs`, so the `/leaving` safety interstitial stays in place for untrusted hosts. Added an `ItemList` JSON-LD to the `/community` index, gated to the indexable canonical page (no filter/page/tab/map — same predicate `generateMetadata` uses to `noindex` slices). | [community/[slug]/\_components/community-listing-jsonld.tsx](../../apps/web/src/app/community/%5Bslug%5D/_components/community-listing-jsonld.tsx), [community/[slug]/page.tsx](../../apps/web/src/app/community/%5Bslug%5D/page.tsx), [community/[slug]/\_components/community-listing-article.tsx](../../apps/web/src/app/community/%5Bslug%5D/_components/community-listing-article.tsx), [community/page.tsx](../../apps/web/src/app/community/page.tsx)                                                                                     |
...
Verification (2026-06-05 breadcrumb + JsonLd-migration bundle): `pnpm typecheck && pnpm lint && pnpm test && pnpm build` all green (typecheck 15/15; lint 0 errors, 3 pre-existing warnings; web 235 tests, 33 files; build 8/8).
