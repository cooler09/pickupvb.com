...
> **Status update (2026-05-24):** Bundle 18 shipped — P3 `BreadcrumbList` JSON-LD added to all four detail pages (events/groups/players/teams) via a shared [`BreadcrumbJsonLd`](../../apps/web/src/app/_components/breadcrumb-jsonld.tsx) component. Renders 3-segment trail (Home → listing → entity) with absolute URLs per spec.
