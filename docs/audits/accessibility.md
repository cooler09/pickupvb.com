...
### Leaflet map has no text alternative or accessible name
...
### Notification popover does not close on Escape
...
### Mobile menu drawer has no focus trap and no Escape close
...
### Touch targets below 44×44 px on primary navigation controls
...
- **Issue:** Both fall short of the 44×44 target size recommendation. Although WCAG 2.5.5 is AAA, these are the *primary* mobile nav controls — users with motor impairments and small touchscreens will struggle.
...
### Focus-ring contrast may fall short on form inputs
...
### "Arrow"-style link affordance is inconsistent with `aria-hidden`
