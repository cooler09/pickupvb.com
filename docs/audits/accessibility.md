...
> convention — and **both are now fixed and `pnpm typecheck && lint && test &&
build` green** (356 web tests). **No P1/P2 this pass.** **C7 (Radix-dialog
> migration) is also now done** — all 6 hand-rolled overlays moved to
> `@radix-ui/react-dialog` / `-popover` (new dep) and the interim
> `useDialogFocusTrap` deleted; quad-green, with a deploy-gated e2e/AT pass on
> the mobile-nav drawer + datetime popover folded into C8. **C8** (shared
> Combobox primitive + end-to-end AT testing) stays open. Full write-up in
...
#### C7. Hand-rolled dialogs/popovers now have a Radix replacement in-tree ✅ (2026-06-09)
...
#### D1. Scoreboard match save-bar status isn't announced; success/error use raw palette ✅ (2026-06-09)
...
#### D2. Public-profile follow/message toggle skips the `aria-pressed` convention + explicit `type` ✅ (2026-06-09)
...
- **C7 — Radix-dialog migration of the hand-rolled overlays. ✅ (2026-06-09)**
  All **6** hand-rolled `role="dialog"` surfaces migrated to Radix and the
  interim `useDialogFocusTrap` (+ its `DIALOG_FOCUSABLE` selector) deleted: the
  scoreboard `ShareModal` + `WinnerOverlay` →
  `@radix-ui/react-dialog` (controlled, `WinnerOverlay` suppresses
  Escape/outside-close since the match is over);
  [notification-bell.tsx](../../apps/web/src/components/notification-bell.tsx)
  - [share-link.tsx](../../apps/web/src/components/share-link.tsx) →
    `@radix-ui/react-popover` (new dep; the share menu drops its `<details>` +
    document-listener hack);
    [datetime-picker.tsx](../../apps/web/src/components/datetime-picker.tsx) →
    `@radix-ui/react-popover` (keeps `id={name}` so the external `<label
htmlFor>` still associates); and
    [mobile-menu.tsx](../../apps/web/src/components/mobile-menu.tsx) →
    `@radix-ui/react-dialog` (Radix `Trigger` toggles, preserving the
    hamburger↔X close affordance; Overlay sits at `z-40` so the header trigger
    stays above the scrim; scroll-lock + focus-trap + Escape + return-focus all
    come from the primitive). New `.md-popover-motion` keyframes added in
    [globals.css](../../apps/web/src/app/globals.css) (mirrors `.md-menu-motion`,
    reads the popover popper transform-origin). All four verify steps green.
    **e2e/AT caveat:** static + quad-green only — the mobile-nav drawer and the
    datetime popover (the two behavior-sensitive surfaces) still want a
    deploy-gated Playwright/VoiceOver pass before final sign-off (folds into C8).
...
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-06-09 | C7: Radix migration of the 6 hand-rolled dialog/popover overlays; deleted `useDialogFocusTrap`                                                        | Migrated all six hand-rolled `role="dialog"` surfaces to Radix and deleted the interim `useDialogFocusTrap` + `DIALOG_FOCUSABLE` selector. **Scoreboard** `ShareModal` + `WinnerOverlay` → controlled `@radix-ui/react-dialog` (portaled; `WinnerOverlay` preventDefaults Escape / pointer-outside since the match is over — no dismiss; both get Radix focus-trap + return-focus + `Title`/`Description` for the accessible name). **notification-bell** + **share-link** → `@radix-ui/react-popover` (added the dep + `pnpm install`; non-modal, so Escape / outside-click close, return-focus and `aria-expanded`/`aria-controls` come free; share-link drops its `<details>` + document-listener hack). **datetime-picker** → `@radix-ui/react-popover`, keeping `id={name}` so the form's external `<label htmlFor>` still associates, with `Popover.Close` for the Done button. **mobile-menu** → `@radix-ui/react-dialog`: Radix `Trigger` toggles (hamburger↔X preserved), `Overlay` at `z-40` keeps the header trigger above the scrim, and scroll-lock + focus-trap + Escape + return-focus all come from the primitive (kept only the route-change-close effect Radix can't know about). Added `.md-popover-motion` keyframes in globals.css mirroring `.md-menu-motion` (reads `--radix-popover-content-transform-origin`). `pnpm typecheck && lint && test && build` green (356 web tests). **Not yet run:** deploy-gated Playwright/VoiceOver on the mobile-nav drawer + datetime popover (folded into C8). | [scoreboard-view.tsx](../../apps/web/src/app/tools/scoreboard/[code]/_components/scoreboard-view.tsx), [notification-bell.tsx](../../apps/web/src/components/notification-bell.tsx), [share-link.tsx](../../apps/web/src/components/share-link.tsx), [datetime-picker.tsx](../../apps/web/src/components/datetime-picker.tsx), [mobile-menu.tsx](../../apps/web/src/components/mobile-menu.tsx), [globals.css](../../apps/web/src/app/globals.css), [package.json](../../apps/web/package.json)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2026-06-09 | D1–D2: scoreboard save-bar status not announced + raw palette; public-profile follow toggle missing `aria-pressed` + explicit `type`                  | 2026-06-09 re-audit remediation (D1–D2; C7–C8 remain open). **D1** — the scoreboard match save-bar's "Saved ✓" / save-error rendered into a plain `<div>` with no live region (silent to AT) and used raw `text-emerald-500` / `text-red-400` (the `-400` red is sub-AA on the white scoreboard theme). Gave the success span `role="status"` and the error span `role="alert"`, and swapped both to the M3 role tokens (`text-md-success` / `text-md-error`). The green "Save final to match" button is a decorative/functional CTA and was left raw per AGENTS.md §17. **D2** — the public-profile `/players/[id]` follow toggle conveyed state by label only; added `aria-pressed={state.isFollowing}` (matching the `players-follow` / `block-control` convention) and `type="button"` to all three buttons (Follow/Unfollow, Message — not in a `<form>`, so no submit bug, just the repo convention). `pnpm typecheck && lint && test && build` green (356 web tests).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | [scoreboard-view.tsx](../../apps/web/src/app/tools/scoreboard/[code]/_components/scoreboard-view.tsx), [player-viewer-actions.tsx](../../apps/web/src/app/players/[id]/_components/player-viewer-actions.tsx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
