...
| [508 / Accessibility](accessibility.md)           | 2026-05-23 | Bundle 41 (2026-05-23) closed the P2 FieldError sweep: all 6 `useFormState` forms now use the shared `FieldError` + `fieldA11y` primitive (`aria-invalid` + `aria-describedby` on each input). Mobile menu focus trap landed in Bundle 2. Remaining: combobox ARIA on address/user pickers, datetime picker Escape, `confirm-submit-button` `aria-modal`, toast focus-ring contrast, "(opens in new tab)" cues, all P3s |
