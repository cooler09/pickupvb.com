...
export class SupabaseHostStripeAccountRepository implements HostStripeAccountRepository {
  ...
  async findByHostId(hostId: string): Promise<HostStripeAccount | null> {
    const { data, error } = await this.client
      ...
      .maybeSingle();
    if (error) {
      throw new Error(`HostStripeAccount.findByHostId(${hostId}) failed: ${error.message}`);
    }
  ...
  async create(account: HostStripeAccount): Promise<void> {
    const { error } = await this.client.from('host_stripe_accounts').insert({
  ...
  async updateStatusByHostId(hostId: string, status: HostStripeAccountStatus): Promise<void> {
