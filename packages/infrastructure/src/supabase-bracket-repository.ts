...
export class SupabaseBracketRepository implements BracketRepository {
    ...
    async findByEventId(eventId: EventId): Promise<Bracket | null> {
        const { data, error } = await this.client
            .from('tournament_brackets')
            .select('*')
            .eq('event_id', eventId)
            .maybeSingle();
        if (error) throw new Error(`bracket findByEventId failed: ${error.message}`);
        if (!data) return null;
        return this.hydrate(data as unknown as BracketRow);
    }
    ...
    private async hydrate(row: BracketRow): Promise<Bracket> {
        ...
        if (matchesRes.error) throw new Error(`bracket matches load failed: ${matchesRes.error.message}`);
    ...
    async listRegisteredTeams(eventId: EventId): Promise<BracketTeamLite[]> {
        const { data, error } = await this.client
            .from('event_teams')
            .select('team_id, teams:teams!inner(name, captain_id)')
            .eq('event_id', eventId);
        if (error) throw new Error(`listRegisteredTeams failed: ${error.message}`);
