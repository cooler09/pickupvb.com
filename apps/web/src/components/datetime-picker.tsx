    ...
    <div>
      <RadixPopover.Root open={open} onOpenChange={setOpen}>
        <RadixPopover.Trigger asChild>
          <button type="button" id={name} className={`${inputClass} text-left`}>
            {mounted ? (
        ...
        <RadixPopover.Portal>
          <RadixPopover.Content
            align="start"
            sideOffset={4}
            className="md-popover-motion border-border-base bg-md-surface-container z-50 rounded-md border p-3 shadow-lg"
          >
            <DayPicker
              mode="single"
            ...
            <div className="border-border-base mt-2 flex items-center gap-2 border-t pt-2">
              ...
              />
              <RadixPopover.Close className={`${primaryButtonClass('sm')} ml-auto`}>
                Done
              </RadixPopover.Close>
            </div>
          </RadixPopover.Content>
