...
export function UserPicker({
  ...
  return (
    <div className="relative space-y-1">
      ...
      {open && (
        <ul className="border-border-base bg-surface absolute z-20 mt-1 max-h-72 w-full overflow-auto rounded-md border shadow-lg">
          {loading && <li className="text-muted px-3 py-2 text-xs">Searching…</li>}
          {!loading && results.length === 0 && query.trim().length >= 2 && (
            <li className="text-muted px-3 py-2 text-xs">No matches.</li>
          )}
          {results.map((r) => (
            <li key={r.id}>
              <button
                ...
                className="hover:bg-fg/5 flex w-full items-center gap-2 px-3 py-2 text-left text-sm"
              >
                {r.avatarUrl ? (
                  <Image
                    src={r.avatarUrl}
