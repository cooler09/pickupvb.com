...
export function GroupCard({ group, action }: { group: GroupCardData; action?: ReactNode }) {
  ...
  const memberLabel = hasCount
    ...
    : '';
  return (
    <li className="border-border-base bg-surface hover:border-primary/40 focus-within:ring-primary/40 rounded-shape-sm relative flex items-start gap-3 border p-3 focus-within:ring-2">
      {group.avatarUrl ? (
        <Image
          src={group.avatarUrl}
          alt=""
