...
export async function GET(
    ...
    try {
        detail = await handlers.getEventDetail.execute(
            new GetEventDetailQuery(id, user.id),
        );
    } catch {
    ...
    const { data: rows } = await admin
        ...
        .select(
            'user_id, joined_at, position, payment_status, amount_paid_cents, payment_intent_id, profiles:profiles!inner(display_name, first_name, last_name)',
        )
        .eq('event_id', id);
    type Row = {
        user_id: string;
