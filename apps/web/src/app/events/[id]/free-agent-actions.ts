 ...
 * Reads optional `notes` from the form.
 */
export async function joinAsFreeAgentFromForm(
    eventId: string,
    formData: FormData,
): Promise<void> {
    ...
    try {
        await handlers.joinEventAsFreeAgent.execute(
            new JoinEventAsFreeAgentCommand(eventId, userId, notes),
        );
    } catch (err) {
...
export async function leaveAsFreeAgent(eventId: string): Promise<void> {
    ...
    try {
        await handlers.leaveEventAsFreeAgent.execute(
            new LeaveEventAsFreeAgentCommand(eventId, userId),
        );
    } catch (err) {
