...
export default async function ReceiptDetailPage({
  ...
  if (event.host_id) {
    // business_name/business_address are not in profiles_public; use admin
    // client so this read works after RLS is tightened to owner-only.
  ...
  const paidCents = rows.filter((r) => r.action === 'paid').reduce((s, r) => s + r.amount_cents, 0);
  ...
  return (
    <section className="space-y-6">
      <div className="flex items-center justify-between gap-3 print:hidden">
        <Link href={'/profile/receipts' as Route} className="text-primary text-sm hover:underline">
          ← All receipts
        </Link>
        <PrintButton />
      </div>

      <article className="border-border-base bg-md-surface-container rounded-shape-sm space-y-6 border p-6 print:border-0 print:p-0">
        ...
        <div className="grid gap-4 text-sm sm:grid-cols-2">
          <div>
            <h2 className="text-muted text-xs font-semibold tracking-wide uppercase">Billed to</h2>
            {buyer?.business_name ? (
              <>
                <p className="mt-1 font-medium">{buyer.business_name}</p>
                {buyer.business_address && (
                  <p className="text-muted whitespace-pre-line">{buyer.business_address}</p>
                )}
                {buyer.tax_id && (
                ...
                <p className="text-muted mt-1 text-xs">
                  Attn: {buyer.display_name ?? user.email ?? 'Account holder'}
                </p>
              </>
            ) : (
              <>
                <p className="mt-1 font-medium">{buyer?.display_name ?? user.email ?? 'You'}</p>
                {user.email && <p className="text-muted">{user.email}</p>}
              </>
            )}
          </div>
          <div>
            <h2 className="text-muted text-xs font-semibold tracking-wide uppercase">Sold by</h2>
            <p className="mt-1 font-medium">{hostBusinessName ?? hostName}</p>
            {hostBusinessName && hostBusinessName !== hostName && (
              <p className="text-muted text-xs">d/b/a {hostName}</p>
            )}
            {hostBusinessAddress && (
              <p className="text-muted whitespace-pre-line">{hostBusinessAddress}</p>
            )}
            <p className="text-muted">via PickupVB (processor: Stripe)</p>
          </div>
        </div>

        <div>
          <h2 className="text-muted text-xs font-semibold tracking-wide uppercase">Event</h2>
          ...
          <p className="text-muted text-sm">{formatDateLong(event.starts_at)}</p>
          {addressLine && <p className="text-muted text-sm">{addressLine}</p>}
        </div>

        <div className="border-border-base overflow-hidden rounded border">
          <table className="w-full text-sm">
            ...
            <tbody>
              <tr className="border-border-base border-t">
                <td className="px-3 py-2">Event signup — {event.title}</td>
                <td className="px-3 py-2 text-right">{formatUsd(paidCents)}</td>
              </tr>
              {refundedCents > 0 && (
                <tr className="border-border-base border-t">
                  <td className="text-muted px-3 py-2">
                    Refund
                    {refundedAt ? ` (${formatDateLong(refundedAt)})` : ''}
                  </td>
                  <td className="text-muted px-3 py-2 text-right">−{formatUsd(refundedCents)}</td>
                </tr>
              )}
              <tr className="border-border-base bg-fg/5 border-t font-semibold">
                <td className="px-3 py-2">Net paid</td>
                <td className="px-3 py-2 text-right">{formatUsd(netCents)}</td>
              </tr>
            </tbody>
          </table>
        </div>

        <footer className="border-border-base text-muted border-t pt-4 text-xs">
          <p>
            Payment processed by Stripe. PickupVB is the platform; the seller of record for the
            event is the host shown above. Tax treatment of this payment is the responsibility of
            the buyer and seller — consult your accountant.
          </p>
          <p className="mt-2">
            Need a corrected receipt? Update your business name, address, or tax ID under{' '}
            <Link href={'/profile/receipts' as Route} className="text-primary hover:underline">
              Receipts → Business info
            </Link>
            , then reprint. For host details, contact the host directly.
          </p>
        </footer>
      </article>
    </section>
