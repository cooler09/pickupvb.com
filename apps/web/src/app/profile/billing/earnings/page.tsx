...
export default async function EarningsPage() {
    ...
    const ytd = transactions.filter(
        (t) => new Date(t.paidAt).getFullYear() === currentYear,
    );
        ...
        <div className="mx-auto max-w-3xl space-y-6 py-4">
            ...
            {transactions.length === 0 ? (
                <div className="rounded-lg border border-border-base bg-surface p-6 text-sm text-muted">
                    No online ticket sales yet. When attendees pay for one of
                    your events through PickupVB, the totals will show up here.
                </div>
            ) : (
                <>
                    {/* ── Totals ──────────────────────────────────────── */}
                    <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <div className="rounded-lg border border-border-base bg-surface p-4">
                            ...
                            <p className="mt-1 text-2xl font-bold">
                                {formatUsd(allTimeTotals.estPayout)}
                            </p>
                            <dl className="mt-2 space-y-0.5 text-xs text-muted">
                                <div className="flex justify-between">
                                    <dt>Gross</dt>
                    ...
                    <p className="text-xs text-muted">
                        Estimated payout = gross − refunds − platform fee.
                        Stripe&apos;s own processing fee (~2.9% + 30¢ per
                        transaction) is deducted separately by Stripe and is
                        not shown here.
                    </p>
