...
/**
 * `Cache-Control` for OG image routes. When a link is shared into Slack /
 * Discord / Twitter the embed unfurlers all fetch the image roughly in
 * parallel — without an explicit cache header that's a thundering-herd
 * cache-miss against Supabase. `public, immutable, max-age=3600,
 * stale-while-revalidate=86400` lets the Vercel edge + downstream CDNs
 * (and the unfurlers themselves) absorb the load. Content keyed off the
 * event/team/etc. row, so an hour of staleness is acceptable for OG
 * previews. See performance audit P3 #11.
 */
const OG_CACHE_CONTROL = 'public, immutable, max-age=3600, stale-while-revalidate=86400';
